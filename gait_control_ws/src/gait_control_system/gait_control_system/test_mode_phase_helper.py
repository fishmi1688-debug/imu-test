...
TEST_MODE_PEAK_SLOPE_EPS = 0.01      # test模式峰值判定的最小斜率变化阈值（rad/采样）
TEST_MODE_PEAK_TIMEOUT_RATIO = 1.60  # 超过当前估计周期的该倍数仍无新峰值，则判定相位失效
...
class TestModePhaseTracker:
    ...
    def __init__(
        ...
        peak_timeout_ratio: float = TEST_MODE_PEAK_TIMEOUT_RATIO,
    ):
        ...
        self.peak_timeout_ratio = max(1.0, float(peak_timeout_ratio))
    ...
    def _deactivate_leg_phase(self, leg_key: str, angle: float | None = None) -> None:
        state = self.phase_state[leg_key]
        if angle is not None:
            state["last_angle"] = float(angle)
        state["last_slope"] = 0.0
        state["last_peak_time"] = None
        state["cycle_start_time"] = None
        state["assist_ready"] = False
        state["trend"] = "unknown"
    ...
    def _update_leg_phase(
        ...
        if peak_detected:
            ...
            accept_peak = last_peak_time is None
            ...
            if accept_peak:
                ...
                # 峰顶附近的小幅回摆可能造成“再次上升后再次下降”，
                # 但只要与上一个有效峰值间隔过短，就不能把它当成新周期起点。
                peak_detected = False
        ...
        last_peak_time = state.get("last_peak_time")
        if last_peak_time is not None:
            peak_timeout_sec = cycle_period * self.peak_timeout_ratio
            if not bool(state.get("assist_ready", False)):
                # 首次建立周期前，允许更慢的踩踏节奏，不要过早把“首周期采样”判成无效。
                peak_timeout_sec = max(float(self.max_cycle_sec), float(peak_timeout_sec))
            if float(current_time) - float(last_peak_time) > peak_timeout_sec:
                self._deactivate_leg_phase(leg_key, angle=angle_now)
                return 0.0, False, peak_detected, False
    ...
    def update(self, left_angle: float, right_angle: float, current_time: float, gait_state: int) -> dict:
        ...
        if int(gait_state) != 1:
            self._deactivate_leg_phase("left", angle=left_angle)
            self._deactivate_leg_phase("right", angle=right_angle)
            self.left_phase_valid = False
            self.right_phase_valid = False
            self.left_assist_ready = False
            self.right_assist_ready = False
